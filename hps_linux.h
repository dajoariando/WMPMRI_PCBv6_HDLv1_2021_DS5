...
#define CPMG_PARALLEL_MULTIFREQ // enable cpmg multiple frequencies operation. This macro added preamp_tuning and set_matching_network inside cpmg_iterate and synchronization signal for multiple child processes. If undefined, then the child process will run in sequence (like a normal cpmg iterate sequence), i.e. one cpmg iterate after another.
...
void handleContinue_child(int sig);
void handleContinue_parent(int sig);
uint8_t parentlock;
...
// magnet addresses
volatile unsigned int *h2p_mgnt_chg_plen_addr = NULL;
volatile unsigned int *h2p_mgnt_chg_dlen_addr = NULL;
volatile unsigned int *h2p_mgnt_dchg_plen_addr = NULL;
volatile unsigned int *h2p_mgnt_dchg_dlen_addr = NULL;
volatile unsigned int *h2p_mgnt_n_addr = NULL;
volatile unsigned int *h2p_mgnt_d_addr = NULL;

volatile unsigned int *h2p_mgnt_hs_addr = NULL;

// fft addresses
volatile unsigned int * h2p_fft_ffpts_addr = NULL;   // the fft number of points address for the fft
volatile unsigned long long * h2p_fft_ctrl_addr = NULL;   // the ram access address for the fft
...
// fft global variable
long * fft_data_re;   // fft real data pointer - should be allocated first
long * fft_data_im;   // fft imag data pointer - should be allocated first
int fftcmd;   // fft command : SAV_ALL_FFT, NO_SAV_FFT, or any positive integer value to take only one FFT data point
